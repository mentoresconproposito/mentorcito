...
export default function MenteeProgress() {
  ...
  var [feedbackDrafts, setFeedbackDrafts] = useState({}); // { [log_id]: texto en edición }
  var [feedbackSaving, setFeedbackSaving] = useState(null); // log_id que se está guardando
  ...
  async function handleGuardarFeedback(logId) {
    var texto = (feedbackDrafts[logId] !== undefined ? feedbackDrafts[logId] : "").trim();
    setFeedbackSaving(logId);
    try {
      await fetch("/api/sheets", {
        method: "POST",
        headers: { "Content-Type": "text/plain" },
        body: JSON.stringify({ action: "save_mentee_feedback", log_id: logId, mentee_email: email, feedback: texto }),
      });
      // Actualizamos el log en memoria, sin recargar todo de nuevo
      setLogs(function (prev) {
        return prev.map(function (l) { return l.log_id === logId ? Object.assign({}, l, { feedback_mentee: texto }) : l; });
      });
      setFeedbackDrafts(function (prev) {
                ...
                <div key={mentorKey}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                    {grupo.logs.map(function (log, i) {
                      ...
                      return (
                        <div key={log.log_id || i} style={{ background: T.card, border: "1px solid " + T.border, borderRadius: 12, padding: "14px 16px" }}>
                          ...
                          {log.proximos_pasos && <div style={{ fontSize: 12.5, color: T.text, marginBottom: 10 }}><strong style={{ color: T.textSub }}>Próximos pasos: </strong>{log.proximos_pasos}</div>}

                          <div style={{ borderTop: "1px solid " + T.border, marginTop: 10, paddingTop: 10 }}>
                            <div style={{ fontSize: 11, color: T.textMuted, marginBottom: 6 }}>Tu feedback de esta sesión</div>
                            <textarea
                              value={feedbackDrafts[log.log_id] !== undefined ? feedbackDrafts[log.log_id] : log.feedback_mentee}
                              onChange={function (e) {
                                var val = e.target.value;
                                setFeedbackDrafts(function (prev) {
                                  var updated = Object.assign({}, prev);
                                  updated[log.log_id] = val;
                                  return updated;
                                });
                              }}
                              placeholder="¿Cómo te resultó esta sesión?"
                              rows={2}
                              style={{ width: "100%", padding: "8px 10px", borderRadius: 8, background: "rgba(255,255,255,0.03)", border: "1px solid " + T.border, color: T.text, fontSize: 12.5, resize: "vertical", boxSizing: "border-box", marginBottom: 6 }}
                            />
                            {feedbackDrafts[log.log_id] !== undefined && feedbackDrafts[log.log_id] !== log.feedback_mentee && (
                              <button
                                onClick={function () { handleGuardarFeedback(log.log_id); }}
                                disabled={feedbackSaving === log.log_id}
                                style={{ padding: "6px 12px", borderRadius: 6, border: "none", background: "linear-gradient(135deg, #4361ee, #7b2ff7)", color: "white", fontWeight: 600, fontSize: 11.5, cursor: "pointer" }}
                              >
                                {feedbackSaving === log.log_id ? "Guardando..." : "Guardar feedback"}
                              </button>
                            )}
                          </div>
                        </div>
